{ config, lib, pkgs, ... }:
let
  cfg = config.cor.home.desktop.hyprland;
in
{
  options.cor.home.desktop.hyprland = {
    enable = lib.cor.mkEnableOption
      false
      "Whether to enable Hyprland configuration for Home Manager."
    ;
    screenTearing = lib.cor.mkEnableOption
      false
      "Whether to enable screen tearing in hyprland (warning, may cause game performance issues)."
    ;

    mainLayout = lib.mkOption {
      default = "dwindle";
      description = "Hyprland's main layout.";
      type = lib.types.nonEmptyStr;
    };


    # TODO: make a function or something to commonify them
    sessionLock = {
      timeout = lib.mkOption {
        type = lib.types.ints.s32;
        description = "Timeout (in seconds) for session lock. Use negative to disable.";
        default = -1;
      };
    };
    ...
    screenOff = {
      timeout = lib.mkOption {
        type = lib.types.ints.s32;
        description = "Timeout (in seconds) for screen off. Use negative to disable.";
        default = -1;
      };
    };
    ...
    suspend = {
      timeout = lib.mkOption {
        type = lib.types.ints.s32;
        description = "Timeout (in seconds) for suspend. Use negative to disable.";
        default = -1;
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    # the single package we add as dependency is polkit
    # other apps must deserve their place here by integrations
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        general = {
          ...
          layout = cfg.mainLayout;
          allow_tearing = cfg.screenTearing;
        };
    ...
    services.hypridle = {
      ...
      settings = {
        general = {
          # lock_cmd and unlock_cmd are set by lock provider (cor.apps.hyprlock for example)
          before_sleep_cmd = "loginctl lock-session"; # lock before suspend.
          after_sleep_cmd = "hyprctl dispatch dpms on"; # to avoid having to press a key twice to turn on the display.
        };

        listener = [
          (lib.mkIf (cfg.sessionLock.timeout > -1) {
            timeout = cfg.sessionLock.timeout;
            on-timeout = "loginctl lock-session";
          })
          (lib.mkIf (cfg.screenOff.timeout > -1) {
            timeout = cfg.screenOff.timeout;
            on-timeout = "hyprctl dispatch dpms off"; # screen off when timeout has passed
            on-resume = "hyprctl dispatch dpms on"; # screen on when activity is detected after timeout has fired.
          })
          (lib.mkIf (cfg.suspend.timeout > -1) {
